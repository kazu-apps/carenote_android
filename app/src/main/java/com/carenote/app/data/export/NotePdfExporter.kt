...
class NotePdfExporter @Inject constructor(
    ...
    override suspend fun export(notes: List<Note>): Uri = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            for (note in notes) {
                ...
                yPos = drawDataRow(canvas, note, yPos)
            }
        ...
        Timber.d("Note PDF exported: ${file.length()} bytes, ${notes.size} records")
    ...
    private fun drawTitle(canvas: Canvas): Float {
        ...
        val title = context.getString(R.string.note_export_title)
